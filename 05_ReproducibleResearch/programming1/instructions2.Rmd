...
xtable(head(activity,4), caption = "First 4 rows of Activity")
```

2- Process/transform the data (if necessary) into a format suitable for your analysis
```{r, comment=FALSE, echo=TRUE}
library(dplyr)
...
```{r, results='asis'}
xtable(head(total_steps,7), caption = "First 7 rows of total steps")
```
...
```{r, results='asis'}
xtable(head(MeanMedianSteps,6), caption = "First 6 rows of mean and median of steps")
```
...
```{r, echo=FALSE, results='asis',fig.fullwidth = TRUE}
...
xtable(intervalsteps[max(intervalsteps$AverageSteps)==intervalsteps$AverageSteps,], caption = "Interval with maximum size of steps")
```
...
```{r, results='asis'}
xtable(head(menactivity,10), caption = "Imputing by mean")
```
...
```{r, results='asis'}
xtable(head(medactivity,10), caption = "Imputing by median")
```
...
```{r, results='asis'}
xtable(head(newactivity,6), caption = "Week additional feature")
```
